...
def format_template(data: dict, tanggal_logout: str, nomor_pelanggan: str, durasi: int, device: str) -> str:
    ...
    if device == "TV":
        pesan = (
            ...
            f"\n"
            f"ⓘ SNK  ⦂\n"
            f"⤷ LOGIN WAJIB PAKE JARINGAN DATA\n"
            f"⤷ LOGOUT JIKA DURASI SEWA SUDAH HABIS.\n"
            f"⤷ ERROR? ESTIMASI PROSES GARANSI 0-3 HARI\n"
            f"⤷ Login 1 device (terpantau)\n"
            f"⤷ Tidak bisa pindah device\n"
            f"⤷ Dilarang otak-atik settingan akun\n"
            f"⤷ 26-30 hari terhitung 1 bulan\n"
            f"⤷ MELANGGAR? DENDA 500RB\n"
            f"━━━━━━━━━━━━━━━━\n"
...
def format_template_plain(data: dict, tanggal_logout: str, nomor_pelanggan: str, durasi: int, device: str) -> str:
    ...
    if device == "TV":
        ...
        pesan = (
            ...
            f"🍿 DATA AKUN NETPLIKS 1P1U 🍿\n"
            f"💌 Email ⦂ {data['email']}\n"
            f"🗝️ Password ⦂ (GUNAKAN KODE MASUK - TIDAK ADA PW)\n"
            f"🔖 Profile ⦂ {data['profil']}\n"
            f"🔒 Pin ⦂ {data['pin']}\n"
            f"⏰ Logout ⦂ {tanggal_logout}\n"
            f" WAJIB LOGOUT TEPAT WAKTU!\n"
