...
def format_template(data: dict, tanggal_logout: str, nomor_pelanggan: str, durasi: int, device: str) -> str:
    ...
    if device == "TV":
        pesan = (
            ...
            f"✿ — durasi langsung berjalan saat akun diterima\n"
            f"✿ — login 1 device, tidak boleh pindah device\n"
            f"✿ — dilarang mengotak-atik settingan apapun\n"
            f"✿ — jika ada notifikasi apapun hubungi seller\n"
            f"✿ — logout tepat waktu jika durasi sudah habis\n"
            f"    • CARA LOGOUT NETFLIX DI TV\n"
            f"1. buka aplikasi netflix di TV\n"
            f"2. pencet tombol berikut di remote TV :\n"
            f"      ⬆️⬆️⬇️⬇️⬅️➡️⬅️➡️⬆️⬆️⬆️⬆️\n"
            f"3. scroll pilihan sampai bertemu opsi keluar\n"
            f"\n"
            f"𝗧𝗵𝗮𝗻𝗸𝘀 𝗳𝗼𝗿 𝗼𝗿𝗱𝗲𝗿, 𝗵𝗮𝗽𝗽𝘆 𝘄𝗮𝘁𝗰𝗵𝗶𝗻𝗴 ♡"
        )
        ...
        pesan = (
            f"ꔘ  NETFLIX 1 Profile 1 User  ꔘ \n"
            f" ﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉\n"
            f"*WAJIB KIRIM BUKTI/SS LOGIN MAX 1x24 JAM.*\n"
            f"\n"
            f"𓄳  🌸. 𝗥𝗘𝗔𝗗 𝗦𝗡𝗞 𝗙𝗜𝗥𝗦𝗧  ☘️📮 ◎\n"
            f"✿ — durasi langsung berjalan saat snk diberikan\n"
            f"✿ — login pakai jaringan data internet / hotspot \n"
            f"✿ — hanya boleh login 1 device (terpantau) \n"
            f"✿ — tidak bisa pindah device, kecuali device rusak\n"
            f"✿ — dilarang mengotak-atik settingan apapun\n"
            f"✿ — dilarang ganti email, password, dan billing\n"
            f"✿ — durasi 1 bulan = 27 - 30 hari \n"
            f"✿ — jika ada notifikasi apapun hubungi seller\n"
            f"✿ — error? estimasi garansi proses max 0 - 3 hari \n"
            f"✿ — changed email / mencuri akun, denda 500K\n"
            f"\n"
            f"💌 Email ⦂ `{data['email']}`\n"
            f"🗝️ Password ⦂ GUNAKAN KODE MASUK - TIDAK ADA KATA SANDI\n"
            f"🔖 Profile ⦂ {data['profil']}\n"
            f"🔒 Pin ⦂ {data['pin']}\n"
            f"⏰ Logout ⦂ {tanggal_logout}\n"
            f"\n"
            f"ⓘ GARANSI HANGUS jika ⦂ \n"
            f"    ⌗ Tidak mematuhi SELURUH SNK yang berlaku\n"
            f"    ⌗ Tidak mengirim bukti login dalam 24 jam\n"
            f"\n"
            f"  𝗧𝗵𝗮𝗻𝗸𝘀 𝗳𝗼𝗿 𝗼𝗿𝗱𝗲𝗿, 𝗵𝗮𝗽𝗽𝘆 𝘄𝗮𝘁𝗰𝗵𝗶𝗻𝗴 ♡🍿"
        )
